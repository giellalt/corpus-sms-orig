...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'https://julkaisut.metsa.fi/assets/pdf/lp/Asarja/a186.pdf'"/>
    ...
    <xsl:variable name="translator_fn" select="'Katri'"/>
    <xsl:variable name="translator_ln" select="'Fofonoff'"/>
    ...
    <!--
        ...
        1, 6-10, 15, 20, 25-30
    -->
    <xsl:variable name="skip_pages" select="'1-7, 9-280'"/>
...
<xsl:template match="body/*">
    ...
    <xsl:element name="p">
        ...
        <xsl:call-template name="globalTextReplace">
            <xsl:with-param name="inputString" select="$text"/>
            <xsl:with-param name="target" select="'³/¥/¡/ˆˆ/ ́/´/′/´/'"/>
            <xsl:with-param name="replacement" select="'ǩ/ǥ/ǧ/ǯǯ/ʹ/ʹ/ʹ/ʹ/'"/>
            <xsl:with-param name="continue" select="0"/>
        </xsl:call-template>
    </xsl:element>
</xsl:template>
