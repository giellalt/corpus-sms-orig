...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://www.samediggi.fi/index.php?option=com_docman&amp;task=doc_download&amp;gid=2789&amp;Itemid=10'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'Tiõrvvʼvuõtt čeäʹppvuõđpeivva puõʹtti'"/>
    <!--
        Information of the first author: first name, last name,
        date or year of birth, nationality.
    -->
    <xsl:variable name="author1_fn" select="'Unna-Maari'"/>
    <xsl:variable name="author1_ln" select="'Pulskai'"/>
    <!-- m, f or unknown -->
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="fin" location="tiedote_su_01_03_2016.pdf"/>
        ...
        <parallel_text xml:lang="sme" location="tiedote_ps_01_03_2016.pdf"/>
        <parallel_text xml:lang="smj" location=""/>
        <parallel_text xml:lang="smn" location="tiedote_as_01_03_2016.pdf"/>
    ...
    <xsl:variable name="bottom_margin" select="'2=62,4=30,7=43'"/>
    ...
    <!--
        ...
        must be defined.
    -->
    <xsl:variable name="inner_right_margin" select="'6=0'"/>
    <xsl:variable name="inner_left_margin" select="'6=0'"/>
    <xsl:variable name="inner_top_margin" select="'6=25'"/>
    <xsl:variable name="inner_bottom_margin" select="'6=18'"/>
...
<xsl:template match="body/*">
    ...
    <xsl:element name="p">
        ...
        <xsl:call-template name="globalTextReplace">
            <xsl:with-param name="inputString" select="$text"/>
            <xsl:with-param name="target" select="'ü/ ́/´/′/´/'"/>
            <xsl:with-param name="replacement" select="'Č/ʹ/ʹ/ʹ/ʹ/'"/>
            <xsl:with-param name="continue" select="0"/>
        </xsl:call-template>
    </xsl:element>
</xsl:template>
